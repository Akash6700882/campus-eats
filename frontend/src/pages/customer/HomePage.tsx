...
const STEPS = [
  ...
  },
  {
    ...
    text: "The moment your payment clears, your order is confirmed and a delivery partner starts getting matched — no waiting on manual approval.",
  },
  {
    number: "03",
    icon: Bike,
...
function HeroShapes() {
  return (
    <div aria-hidden className="pointer-events-none absolute inset-0 overflow-hidden">
      <svg className="absolute -left-8 top-10 h-24 w-24 text-primary/40 sm:left-4" viewBox="0 0 100 100" fill="none">
        <circle cx="50" cy="50" r="46" stroke="currentColor" strokeWidth="6" />
      </svg>
      <svg className="absolute right-2 top-24 h-16 w-16 text-gold/60 sm:right-16" viewBox="0 0 100 100" fill="none">
        <circle cx="50" cy="50" r="46" stroke="currentColor" strokeWidth="8" />
      </svg>
      <div className="absolute left-1/4 top-4 h-3 w-3 rounded-full bg-primary/50" />
      <div className="absolute right-1/3 bottom-6 h-2 w-2 rounded-full bg-gold/70" />
      <div className="absolute left-10 bottom-10 h-4 w-4 rounded-full bg-success/40" />
    </div>
  );
}
  ...
  return (
    <div>
      <section className="relative overflow-hidden bg-deep text-deep-foreground">
        ...
        <div className="container relative flex flex-col items-center gap-6 py-16 text-center sm:py-24">
          ...
          >
            ...
            <h1 className="max-w-2xl text-balance font-heading text-5xl font-black tracking-tight sm:text-6xl lg:text-7xl">
              We don't just cook,{" "}
              <span className="text-primary">we deliver campus favourites.</span>
            </h1>
            <p className="max-w-lg text-balance text-deep-foreground/70">
          ...
          <form onSubmit={handleSearch} className="flex w-full max-w-lg items-center gap-2">
            <div className="relative flex-1">
              ...
              <Input
                ...
                className="h-12 border-transparent bg-background pl-9 text-foreground"
              />
            </div>
          ...
          <div className="mt-2 flex items-center gap-8 sm:gap-12">
            {STATS.map((stat) => (
              <div key={stat.label} className="flex flex-col items-center">
                <span className="font-heading text-2xl font-extrabold text-primary sm:text-3xl">{stat.value}</span>
                <span className="text-xs text-deep-foreground/60">{stat.label}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="container py-12">
        <h2 className="mb-8 text-center font-heading text-3xl font-extrabold">How it works</h2>
        <div className="grid gap-8 sm:grid-cols-3">
          {STEPS.map((step) => (
            <div key={step.number} className="relative rounded-2xl bg-card p-6 shadow-sm ring-1 ring-foreground/10">
              <span className="font-heading text-4xl font-extrabold text-primary/15">{step.number}</span>
              <span className="mt-2 flex h-10 w-10 items-center justify-center rounded-xl bg-primary/10">
                <step.icon className="h-5 w-5 text-primary" />
              </span>
              <h3 className="mt-3 font-heading text-base font-bold">{step.title}</h3>
      ...
      <section className="container pb-16">
        <div className="relative overflow-hidden rounded-2xl bg-deep px-6 py-12 text-center text-deep-foreground">
          ...
          <div aria-hidden className="pointer-events-none absolute -bottom-12 -left-8 h-32 w-32 rounded-full bg-primary/20" />
          <h2 className="relative font-heading text-2xl font-extrabold">Hungry for something specific?</h2>
